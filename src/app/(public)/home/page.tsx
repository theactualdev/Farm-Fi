    ...
    <div className="wrapper bg-[var(--background)] text-white">
      ...
      >
        <div className="text text-white flex flex-col items-start max-w-[506px] space-y-5">
          <h2 className={`${ibm.className} font-semibold text-[38px]`}>
            Trading Agricultural Produce Made Seamless with SUI
          </h2>
          ...
          <Link
            className={`flex ${lora.className} font-medium py-[18px] px-5 bg-[var(--secondary)] rounded-[40px] gap-6`}
            href={"/signup"}
          >
            {" "}
            Get Started{" "}
      ...
      <section
        id="stats"
        className="grid md:grid-cols-4 p-16 place-items-center"
      >
      ...
      <section id="join" className="mx-[169px] space-y-10">
        <div className="title flex flex-col items-center justify-center space-y-16">
          <h3 className={`${ibm.className} text-[30px]`}>
            Become a Part of FarmFi
          </h3>
          <p className={`${lora.className}`}>
            Want to Trade on FarmFi, we have just what you need.
          </p>
        </div>
            ...
            return (
              <div className="md:max-w-[45%] space-y-[30px]" key={index}>
                ...
                <div className="text-container space-y-5">
                  <h4 className={`${ibm.className} text-[25px]`}>
                    {" "}
                    {card.title}{" "}
                  </h4>
      ...
      <section id="offer" className="p-16 space-y-16">
        <h2 className={`capitalize ${ibm.className} text-[30px] text-center`}>
          What we offer
        </h2>
        ...
        <div className={`${lora.className} flex justify-center space-x-4`}>
          <Link
            className="bg-[var(--secondary)] font-semibold py-3 px-4 rounded-[40px]"
            href={"/"}
          >
            Join Us Today!
          </Link>
          <Link
            className="text-[var(--background)] bg-white font-semibold py-3 px-4 rounded-[40px]"
            href={"/"}
          >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {feedback.map((feed, index) => {
            return (
              <div
                key={index}
                className="p-4 rounded-xl border border-[#B3B3B3] flex flex-col gap-3"
              >
                <div className="head flex items-center gap-3">
                  <Image
                    ...
                    width={32}
                    height={32}
                    alt={`${feed.name}'s feedback`}
                  />
                  <p className={`${lora.className} font-medium text-lg`}>
                    {" "}
                    {feed.name}{" "}
                  </p>
                </div>
                <p className={`${lora.className} font-normal text-xl`}>
                  {" "}
                  {feed.feed}{" "}
                </p>
              </div>
